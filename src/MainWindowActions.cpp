...
void MainWindow::importSubtitle() {
  QString fileName = QFileDialog::getOpenFileName(this,
      tr("Import Subtitles"), "",
      tr("Subtitle Files (*.srt *.ass *.ssa *.vtt *.sub);;All Files (*)"));
  ...
  if (!fileName.isEmpty()) {
    ...
    // Clear the menu so updateUI() repopulates it with the new track
...
// Add these helper functions to your MainWindow class:

void MainWindow::clearImportedCheckmarks() {
    for (auto *row : m_subtitleMenu->findChildren<SubtitleImports*>()) {
        row->setChecked(false);
    }
}

void MainWindow::clearImportedCheckmarksExcept(int exceptId) {
    for (auto *row : m_subtitleMenu->findChildren<SubtitleImports*>()) {
        if (row->getId() != exceptId) {  // You'll need to add getId() to SubtitleImports
            row->setChecked(false);
        }
    }
}
